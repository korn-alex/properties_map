...
simpleheat.prototype = {
    ...
    radius: function (zoom, blur) {
        ...
        var circle = this._circle = this._createCanvas(),
            ...
            r = this.defaultRadius * Math.pow(2, zoom - 15),
            blurredR = this._r = r;// + blur;
    ...
    draw: function (zoom, minOpacity) {
        ...
        for (var i = 0, len = this._data.length, p; i < len; i++) {
            p = this._data[i];
            ctx.globalAlpha = p[2]; //Math.max(p[2] / this._max, minOpacity === undefined ? 0.05 : minOpacity);
...
L.HeatLayer = (L.Layer ? L.Layer : L.Class).extend({
    ...
    _redraw: function () {
        ...
        this._heat.data(data).draw(this._map.getZoom(), this.options.minOpacity);
